...
class Bunny {
    ...
    mul() {
        ...
        if (newCell && this.multiply >= 10) {
            ...
            var newBunny = new Bunny(newX, newY);
            bunnyArr.push(newBunny);
            this.multiply = 0;
        }
    }
    ...
    die() {
        matrix[this.y][this.x] = 0;
        for (var i in bunnyArr) {
            if (this.x == bunnyArr[i].x && this.y == bunnyArr[i].y) {
                bunnyArr.splice(i, 1);
                break;
            }
        }
...
class Lion {
    ...
    eat() {
        var emptyCells = this.chooseCell(3, 4)
        var newCell = emptyCells[Math.floor(Math.random() * emptyCells.length)]
        var emptyCells1 = this.chooseCell(6)
        var newCell1 = emptyCells1[Math.floor(Math.random() * emptyCells1.length)]
        ...
        if (newCell) {
            ...
            for (var i in bunnyArr) {
                if (newX == bunnyArr[i].x && newY == bunnyArr[i].y) {
                    bunnyArr.splice(i, 1)
                    break
                }
            }
